...
const i18n = {
  vi: {
    nav_home: "Trang Chủ", nav_corp: "Thuê Xe Doanh Nghiệp", nav_short: "Thuê Xe Ngắn Hạn", nav_practice: "Bài Thực Hành", nav_contact: "Liên Hệ",
    practice_badge: "Practice", practice_title: "Bài Thực Hành", practice_subtitle: "Tổng hợp các bài lab HTML, CSS và Javascript — nhấn từng mục để mở bài làm", practice_btn: "Xem chi tiết",
    ...
    agree_before: "Tôi đồng ý với ", agree_policy: "Chính sách bảo vệ dữ liệu cá nhân", submit: "Gửi Yêu Cầu Tư Vấn",
    sending: "Đang gửi...",
  ...
  en: {
    nav_home: "Home", nav_corp: "Corporate Rental", nav_short: "Short-term Rental", nav_practice: "Practice Labs", nav_contact: "Contact",
    practice_badge: "Practice", practice_title: "Practice Exercises", practice_subtitle: "HTML, CSS and JavaScript lab exercises — click each item to open", practice_btn: "View details",
    ...
    agree_before: "I agree to the ", agree_policy: "Personal Data Protection Policy", submit: "Submit Request",
    sending: "Sending...",
  ...
  kr: {
    nav_home: "홈", nav_corp: "기업 장기 렌탈", nav_short: "단기 렌탈", nav_practice: "실습 과제", nav_contact: "연락처",
    practice_badge: "Practice", practice_title: "실습 과제", practice_subtitle: "HTML, CSS, JavaScript 랩 과제 — 항목을 클릭하여 열기", practice_btn: "자세히 보기",
    ...
    agree_before: "", agree_policy: "개인정보 보호정책에 동의합니다", submit: "상담 신청",
    sending: "전송 중...",
...
document.addEventListener('DOMContentLoaded', () => {
  if (window.appReady) bootApp();
  else document.addEventListener('app:ready', bootApp, { once: true });
});
...
function renderPage() {
  ...
  setText('#nav-practice', d.nav_practice);
  setText('#nav-contact', d.nav_contact);
  setText('#nav-mobile-home', d.nav_home);
  setText('#nav-mobile-corp', d.nav_corp);
  setText('#nav-mobile-short', d.nav_short);
  setText('#nav-mobile-practice', d.nav_practice);
  setText('#nav-mobile-contact', d.nav_contact);
  ...
  // Practice
  setText('#practice-badge', d.practice_badge);
  setText('#practice-title', d.practice_title);
  setText('#practice-subtitle', d.practice_subtitle);
  document.querySelectorAll('.lab-card-btn').forEach(btn => { btn.textContent = d.practice_btn; });
...
function setText(sel, val) {
  ...
  if (el.tagName === 'SPAN' && !val.includes('<')) {
    el.textContent = val;
    return;
  }
  el.innerHTML = val;
...
function renderAgreeLabel(d) {
  const el = document.getElementById('agree-text');
  if (!el) return;
  if (d.agree_before) {
    el.innerHTML = `${d.agree_before}<button type="button" class="form-policy-link" data-privacy>${d.agree_policy}</button>`;
  } else {
    el.innerHTML = `<button type="button" class="form-policy-link" data-privacy>${d.agree_policy}</button>`;
  }
}
...
function initScroll() {
  ...
  const onScroll = () => {
    ...
    const onHome = document.body.classList.contains('page-home-active');
    if (header) header.classList.toggle('scrolled', !onHome || sy > 20);
...
/* ===== Privacy Modal ===== */
function openPrivacyModal(isTerms = false) {
...
function initPrivacyModal() {
  ...
  document.addEventListener('click', (e) => {
    const btn = e.target.closest('[data-privacy], [data-terms]');
    if (!btn) return;
    e.preventDefault();
    e.stopPropagation();
    openPrivacyModal(btn.hasAttribute('data-terms'));
  });
